...
var pipe = function(spec, my) {
  ...
  my.server = spec.server || my.cfg['PIPED_SERVER'];
  my.port = spec.port || my.cfg['PIPED_PORT'];
  ...
  register = function(filter, router, cont_) {
    ...
    ctx.on('finalize', function() {
	     cont_(new Error('Connection Error'));
	     delete my.ctx;
	   });
